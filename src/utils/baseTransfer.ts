...
export const transferFiles = async (
	...
	const moveResults: MoveResults = { succeeded: [], failed: [] };
	...
	const movables = (
		await Promise.all(
			srcFilePaths.map(async (dirent: Dirent) => {
	...
	if (movables.length === 0) return { result: moveResults, err: null };
	...
	for (const entry of movables) {
		...
		try {
			...
			moveResults.failed.push({ srcFilePath, dstFilePath, err: e as Error });
			continue;
		}
 ...
 */
export const getMovableDirents = async (
	srcDir: string,
	dstDir: string,
	dirent: Dirent,
	mode: Mode,
...
export const delSrc = async (srcPath: string, dirent: Dirent) => {
