...
describe("Fn: getFilePaths", () => {
	test("01. invalid directory path returns 'directory path validation failed'", async () => {
		const srcDir = "";
		await expect(dirl.getFilePaths(srcDir)).rejects.toThrow(/Directory path validation failed/);
	});
	...
	test("04. Valid and existing dir path resolves promise", async () => {
		...
		expect(await dirl.getFilePaths(srcDir)).toHaveLength(12);
	});

	test("05. Using dirNameFilter=level2 returns 4 filePaths", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFilePaths(srcDir, { dirNameFilter: "level4" })).toHaveLength(6);
	});

	test("06. Using dirNameFilter=level9 returns 0 filePaths", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFilePaths(srcDir, { dirNameFilter: "level9" })).toHaveLength(0);
	});
	test("07. Using fileNameFilter=5 returns 1 filePath", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFilePaths(srcDir, { fileNameFilter: "5" })).toHaveLength(3);
	});
	test("08. Using fileNameFilter=test returns 0 filePath", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFilePaths(srcDir, { fileNameFilter: "test" })).toHaveLength(0);
	});
	test("09. Using extNameFilter=jpg returns 0 filePath", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFilePaths(srcDir, { extNameFilter: "jpg" })).toHaveLength(0);
	});
	test("10. Using extNameFilter=txt returns 12 filePath", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFilePaths(srcDir, { extNameFilter: "txt" })).toHaveLength(12);
	});
	test("11. /empty return 0 filePaths", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\empty";
		expect(await dirl.getFilePaths(srcDir)).toHaveLength(0);
	});
});
...
describe("Fn: getFileCount", () => {
	test("12. invalid directory path throws 'directory path validation failed'.", async () => {
		const srcDir = "";
		await expect(dirl.getFileCount(srcDir)).rejects.toThrow(/Directory path validation failed/);
	});
	...
	test("15. /10files returns 12", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFileCount(srcDir)).toBe(12);
	});

	test("16. Using dirNameFilter=level4 returns 6", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFileCount(srcDir, { dirNameFilter: "level4" })).toBe(6);
	});

	test("17. Using dirNameFilter=level9 returns 0", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFileCount(srcDir, { dirNameFilter: "level9" })).toBe(0);
	});
	test("18. Using fileNameFilter=5 returns 3", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFileCount(srcDir, { fileNameFilter: "5" })).toBe(3);
	});
	test("19. Using fileNameFilter=test returns 0", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFileCount(srcDir, { fileNameFilter: "test" })).toBe(0);
	});
	test("20. Using extNameFilter=jpg returns 0", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFileCount(srcDir, { extNameFilter: "jpg" })).toBe(0);
	});
	test("21. Using extNameFilter=txt returns 12", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\10files";
		expect(await dirl.getFileCount(srcDir, { extNameFilter: "txt" })).toBe(12);
	});
	test("22. srcDir=/empty return 0", async () => {
		const srcDir = "C:\\dev\\sandbox\\files\\empty";
		expect(await dirl.getFileCount(srcDir)).toBe(0);
	});
});
