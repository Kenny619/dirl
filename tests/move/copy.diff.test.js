...
const root = "./tests/testFiles/copy/diff/src";
const newRoot = "./tests/testFiles/copy/diff/dst";
...
describe("move.diff() - scrDir input", async () => {
	...
	it("scrDir = Valid absolute path to a directory resolves to truthy value", async () => {
		await expect(dirl.copy.diff(path.resolve(root, "./absolutePathSrc"), newRoot)).resolves.toBeTruthy();
...
describe("move.diff() - newRoot input", async () => {
	...
	for (const [type, value] of Object.entries(expectedInputType("string"))) {
		it(`newRoot = ${type} throws error`, async () => {
			await expect(dirl.copy.diff(root, value)).rejects.toThrowError();
		});
	}
	...
	it("newRoot = Valid path to a file resolves to truthy value", async () => {
		await expect(
			dirl.copy.diff(path.join(root, "/relativePathDst"), path.join(newRoot, "/relativePathDst")),
		).resolves.toBeTruthy();
	...
	it("newRoot = Valid absolute path to a directory resolves to truthy value", async () => {
		await expect(
			dirl.copy.diff(path.join(root, "absolutePathDst"), path.resolve(newRoot, "./absolutePathDst")),
		).resolves.toBeTruthy();
...
describe("move.diff() - filter", async () => {
	it("filter = {ext:rar} fails to copy 1 file", async () => {
		const res = await dirl.copy.diff(root, newRoot, { ext: "rar" });
	...
	it("filter = {file: tier1} fails to copy 3 files", async () => {
		const res = await dirl.copy.diff(root, newRoot, { file: "tier1" });
		expect(res.succeeded).toHaveLength(0);
		expect(res.failed).toHaveLength(0);
	...
	it("filter = {dir:empty } fails to copy 2 directories", async () => {
		const res = await dirl.copy.diff(root, newRoot, { dir: "empty" });
		expect(res.succeeded).toHaveLength(0);
		expect(res.failed).toHaveLength(0);
	...
	it("filter = {dir:filterTest, file:log} copies 1 file", async () => {
		const res = await dirl.copy.diff(newRoot, root, { dir: "filterTest", file: "log" });
	...
	it("filter = {dir:filterTest, ext:png} copies 1 file", async () => {
		const res = await dirl.copy.diff(newRoot, root, { dir: "filterTest", ext: "png" });
	...
	it("filter = {file:filterTest, ext:wma} copies 1 file", async () => {
		const res = await dirl.copy.diff(newRoot, root, { file: "audio", ext: "wma" });
	...
	it("filter = {file:filterTest, file:zipped, ext:zip} copies 1 file", async () => {
		const res = await dirl.copy.diff(newRoot, root, { dir: "filterTest", file: "^zip", ext: "zip" });
...
// 		const res = await dirl.copy.diff(root, newRoot);
