...
//input type
const root = "./tests/testFiles/flatten/unique/src";
const dstDir = "./tests/testFiles/flatten/unique/dst";
const separatorDstDir = "./tests/testFiles/flatten/unique/separatorDstDir";
const filterDstDir = "./tests/testFiles/flatten/unique/filterDstDir";
...
describe("flatten.all() - scrDir input", async () => {
	...
	it("scrDir = Valid absolute path to a file throws error", async () => {
		await expect(
			dirl.flatten.unique(path.join(root, "/Users/kenny/dev/dirl/tests/testFiles/movescopy.log"), dstDir),
		).rejects.toThrowError();
	});

	//valid path resolves to truthy value
	it("scrDir = Valid directory path resolves to truthy value", async () => {
		await expect(dirl.flatten.unique(path.join(root, "/relativePathSrc"), dstDir)).resolves.toBeTruthy();
	});
...
describe("flatten.all() - dstDir input", async () => {
	...
	for (const [type, value] of Object.entries(expectedInputType("string"))) {
		it(`dstDir = ${type} throws error`, async () => {
			await expect(dirl.flatten.unique(root, value)).rejects.toThrowError();
		});
	}

	//valid path resolves to truthy value
	it("dstDir = Valid path to a file resolves to truthy value", async () => {
		await expect(dirl.flatten.unique(path.join(root, "/relativePathDst"), dstDir)).resolves.toBeTruthy();
	});

	//valid relative path resolves to truthy value
	it("dstDir = Valid absolute path to a directory resolves to truthy value", async () => {
		await expect(dirl.flatten.unique(path.join(root, "absolutePathDst"), dstDir)).resolves.toBeTruthy();
	});
});
...
describe("flatten.all() - separator input", async () => {
	...
	for (const symbol of invalidSeparators()) {
		it(`separator = ${symbol} throws error`, async () => {
			await expect(dirl.flatten.unique(path.join(root, "valiDir"), dstDir, symbol)).rejects.toThrowError();
		});
	}
	...
	for (const symbol of validSeparators()) {
		it(`separator = ${symbol} resolves to truthy value`, async () => {
			await expect(dirl.flatten.unique(path.join(root, "validDir"), separatorDstDir, symbol)).resolves.toBeTruthy();
		});
	}
});
...
describe("flatten.all() - filter", async () => {
	it("filter = {ext:rar} fails to copy 1 file", async () => {
		await expect(dirl.flatten.unique(root, filterDstDir, "_", { ext: "rar" })).resolves.toBeTruthy();
		expect(await dirl.get.fileCount(filterDstDir, { ext: "rar" })).toBe(1);
	});

	it("filter = {file: tier1} fails to copy 3 files", async () => {
		const res = await dirl.flatten.unique(root, filterDstDir, "_", { file: "tier1" });
		expect(await dirl.get.fileCount(filterDstDir, { file: "tier1" })).toBe(3);
	});

	it("filter = {dir:empty } fails to copy 2 directories", async () => {
		await expect(dirl.flatten.unique(root, filterDstDir, "_", { dir: "empty" })).resolves.toBeTruthy();
	});

	it("filter = {dir:filterTest, file:log} copies 1 file", async () => {
		await expect(
			dirl.flatten.unique(root, filterDstDir, "_", { dir: "filterTest", file: "log" }),
		).resolves.toBeTruthy();
	...
	it("filter = {dir:filterTest, ext:png} copies 1 file", async () => {
		await expect(dirl.flatten.unique(root, filterDstDir, "_", { dir: "filterTest", ext: "png" })).resolves.toBeTruthy();
	...
	it("filter = {file:filterTest, ext:wma} copies 1 file", async () => {
		await expect(dirl.flatten.unique(root, filterDstDir, "_", { file: "audio", ext: "wma" })).resolves.toBeTruthy();
	...
	it("filter = {file:filterTest, file:zipped, ext:zip} copies 1 file", async () => {
		await expect(
			dirl.flatten.unique(root, filterDstDir, "_", { dir: "filterTest", file: "^zip", ext: "zip" }),
		).resolves.toBeTruthy();
...
describe("flatten.all() - all", async () => {
	it("srcDir = dstDir when all files are copied", async () => {
		const res = await dirl.flatten.unique(root, dstDir);
