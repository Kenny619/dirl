...
class DirlMove extends DirlGet {
    ...
    constructor() {
        super();
        this.move = {
            overwrite: async (srcDir, dstDir, filters = {}) => {
                try {
            ...
            diff: async (srcDir, dstDir, filters = {}) => {
                try {
            ...
            ifNew: async (srcDir, dstDir, filters = {}) => {
                try {
    ...
    async transferDir(srcDir, dstDir, mode, filters) {
        try {
    ...
    printTransferResult(result, mode) {
    ...
    async transferFiles(srcDir, dstDir, filters, mode) {
        //Validate Dir
        try {
        ...
        try {
            isFileMovableCheckResults = await Promise.all(dstFilePaths.map((dstFile, index) => super.isFileMovable(srcFilePaths[index], dstFile, mode)));
        }
        catch (e) {
        ...
        for (let i = 0; i < srcFilePaths.length; i++) {
            ...
            if (!isFileMovableCheckResults[i])
                continue;
