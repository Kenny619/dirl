...
class DirlMove {
    ...
    async overwrite(srcDir, dstDir, filters = {}) {
        const { data, err } = await transferFiles(srcDir, dstDir, "moveOverwrite", filters);
        if (err !== null)
            throw err;
        return data;
    }
    ...
    async diff(srcDir, dstDir, filters = {}) {
        const { data, err } = await transferFiles(srcDir, dstDir, "moveDiff", filters);
        if (err !== null)
            throw err;
        return data;
    }
    ...
    async ifNew(srcDir, dstDir, filters = {}) {
        const { data, err } = await transferFiles(srcDir, dstDir, "moveIfNew", filters);
        if (err !== null)
            throw err;
        return data;
    }
