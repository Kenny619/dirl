 ...
 */
export const applyFilter = (dirents, regexFilters) => {
    ...
    return dirents.filter((dirent) => {
        ...
        if (dirent.isFile()) {
            if (ignoreFiles.includes(dirent.name))
                return false;
 ...
 */
export const createRegexFilters = (filters) => {
    ...
    if (!filters.dir && !filters.file && !filters.ext)
        return null;
    return Object.keys(filters).reduce((acc, key) => {
        if (!["dir", "file", "ext"].includes(key))
            return acc;
        try {
            acc[key] = new RegExp(filters[key]);
        }
        catch (e) {
            throw e;
        }
