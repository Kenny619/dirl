 ...
 */
export const validateSrcDir = async (srcDir) => {
    try {
        const stat = await fsp.stat(srcDir);
        if (!stat.isDirectory())
            throw new Error(`${srcDir} is not a directory.  Please provide a directory path.`);
        if (stat.isDirectory())
            await fsp.access(srcDir, fsp.constants.R_OK);
    }
    catch (e) {
        throw e;
    }
 ...
 */
export const validateDstDir = async (dstDir) => {
    try {
    ...
    try {
        ...
        if (!/ENOENT|EPERM/.test(e.message))
            throw new Error(`Failed to unlink test file in ${dstDir} ${e.message} ${e.stack}`);
    }
    return true;
};
/**
 ...
 */
export const validatetDirs = async (srcDir, dstDir) => {
    if (srcDir === dstDir)
